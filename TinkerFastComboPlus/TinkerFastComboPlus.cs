...
{
    [ExportPlugin(name: "TinkerFastComboPlus", author: "YEEEEEEE", version: "2.1.0.0", units: HeroId.npc_dota_hero_tinker)]
    internal class TinkerFastComboPlus : Plugin
    {
        ...
        private void ComboEngine(EventArgs args)
        {
            ...
            var talent10 = Owner.Spellbook.Spells.First(x => x.Name == "special_bonus_cast_range_75");
            if (talent10.Level > 0)
            {
        ...
        private void Information(EventArgs args)
        {
			...
			if (Menu.Item("Calculator").GetValue<bool>())
			{
                ...
                if (Menu.Item("debug").IsActive())
                {
                    ...
                    Drawing.DrawText(Owner.Distance2D(targetInf).ToString(), new Vector2(HUDInfo.ScreenSizeX() / 2 + 2 + coordX + 420, HUDInfo.ScreenSizeY() / 2 + 435 + 2 + coordY), new Vector2(30, 200), Color.Black, FontFlags.AntiAlias);
                    Drawing.DrawText(Owner.Distance2D(targetInf).ToString(), new Vector2(HUDInfo.ScreenSizeX() / 2 + coordX + 420, HUDInfo.ScreenSizeY() / 2 + 435 + coordY), new Vector2(30, 200), Color.LimeGreen, FontFlags.AntiAlias);
        ...
        private float GetLaserDamage()
        {
            ...
            var talent25 = Owner.Spellbook.Spells.First(x => x.Name == "special_bonus_unique_tinker");
            if (talent25.Level > 0)
            {
            ...
            var talent10 = Owner.Spellbook.Spells.First(x => x.Name == "special_bonus_spell_amplify_6");
            if (talent10.Level > 0)
            {
                totalSpellAmp += (talent10.AbilitySpecialData.First(x => x.Name == "value").Value) / 100.0f;
            }

            var trident = Owner.Inventory.Items.FirstOrDefault(x => x.Id == AbilityId.item_trident);
            if (trident != null)
            {
